/*---------------------------------------------------------------------------------
    ...
    Dan1 & Dan2 & Dan3 compression support from Amy Purple aka newcoleco
...
int Convert2Pic(char *filebase, unsigned char *buffer, unsigned int *tilemap, unsigned int *palet, unsigned int *tilecol, int num_tiles, int mapw, int maph, int savemap, int savepal)
{
	...
	if (compress==0) { // no compression
		...
		if (ecmmode>1)
		{
			lenencodet1=pleCompress( tiMem,tiMemEncode,lenencode);
			lenencodet2=pleCompress( tiMem+256*8,tiMemEncode+256*8,lenencode);
			lenencodet3=pleCompress( tiMem+256*8*2,tiMemEncode+256*8*2,lenencode);
		}
		else
			lenencode=pleCompress( tiMem,tiMemEncode,lenencode);
		if (gramode==1)
		{
			memcpy(coMemEncode,coMem,32);
			lenencode1=32;
			...
			lenencode1=pleCompress(coMem,coMemEncode,lenencode1);
        if (savemap)
            lenencode2=pleCompress(maMem,maMemEncode,lenencode2);
        else
            memcpy(maMemEncode,maMem,lenencode2);
	}
	...
	else if (compress==4) { // dan2 compression
		if (ecmmode>1)
		{
			lenencodet1=dan2Compress( tiMem,tiMemEncode,lenencode);
			lenencodet2=dan2Compress( tiMem+256*8,tiMemEncode+256*8,lenencode);
			lenencodet3=dan2Compress( tiMem+256*8*2,tiMemEncode+256*8*2,lenencode);
		}
		else
        {
			lenencode=dan2Compress( tiMem,tiMemEncode,lenencode);
        }
		if (gramode==1)
		{
			memcpy(coMemEncode,coMem,32);
        ...
        {
			lenencode1=dan2Compress(coMem,coMemEncode,lenencode1);
        }
        if (savemap)
        {
...
void PrintOptions(char *str) {
	...
	printf("\n\nOptions are:");
	printf("\n\n--- General options ---");
	printf("\n-c[no|rle|ple|dan1|dan2|dan3]    Compression method [no]");
	printf("\n\n--- Graphic options ---");
	...
	for(i=1;i<argc;i++)	
	{
		if(arg[i][0]=='-') 
		{
			...
			else if(arg[i][1]=='c')		//compression method
			{
				...
				else if( strcmp(&arg[i][1],"cdan1") == 0)
				{
					compress = 3; // DAN1 compression
				}
				else if( strcmp(&arg[i][1],"cdan2") == 0)
				{
